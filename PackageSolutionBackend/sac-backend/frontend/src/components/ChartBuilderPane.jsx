...
import './ChartBuilderPane.css'; // We will create this
...
const getRules = (type) => {
  ...
  // Default rules for standard charts (Bar, Column, Line, Area, etc.)
  // While technically unlimited in SAC, 5 measures and 2 dimensions is the practical limit before the chart becomes unreadable.
  return { maxMeas: 5, maxDim: 2, minMeas: 1, minDim: 1 }; 
};

const ChartBuilderPane = ({ item, isWidget, availableMetadata, onChange }) => {
  ...
  // If the structure is legacy (flat arrays), we fall back, otherwise we pick by sourceModel
  ...
  const handleModelChange = (e) => {
    ...
    // Clear bindings when switching models as they likely don't apply anymore
  ...
  return (
    <div className="builder-pane-ui" onClick={(e) => e.stopPropagation()}>
      <div className="builder-header-ui">
        <Settings2 size={16} /> Data Binding Builder
      </div>

      {isMultiModel && availableModels.length > 0 && (
        <div className="builder-section" style={{ marginBottom: '12px' }}>
          <div className="builder-section-title">Source Model</div>
          <select 
            value={sourceModel} 
            onChange={handleModelChange}
            style={{ width: '100%', padding: '6px', borderRadius: '4px', border: '1px solid var(--border-color)', fontSize: '0.85rem' }}
          >
          ...
          <div className="builder-pill-container">
            {currentDims.map(d => (
              <div key={d} className="builder-pill">
                <span>{d}</span>
                <button className="remove-btn" onClick={() => handleRemoveDim(d)}><X size={14} /></button>
              </div>
            ))}
          </div>
              ...
              {showAddDim && (
                <div className="add-dropdown">
                  <input 
                    ...
                    style={{ width: '100%', padding: '6px', boxSizing: 'border-box', border: 'none', borderBottom: '1px solid #e2e8f0', outline: 'none', fontSize: '0.8rem' }}
                    autoFocus
                  />
                  ...
                  {currentAvailableDims.filter(d => !currentDims.includes(d) && d.toLowerCase().includes(dimSearch.toLowerCase())).length === 0 && (
                    <div className="add-dropdown-item" style={{ color: '#94a3b8', cursor: 'default' }}>No match</div>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="builder-pill-container">
            {currentMeas.map(m => (
              <div key={m} className="builder-pill measure-pill">
                <span>{m}</span>
                <button className="remove-btn" onClick={() => handleRemoveMeas(m)}><X size={14} /></button>
              </div>
            ))}
          </div>
          {currentMeas.length < rules.maxMeas && (
            <div className="add-container" ref={measDropdownRef}>
              <button className="add-btn warning" onClick={() => setShowAddMeas(!showAddMeas)}>
...
export default ChartBuilderPane;
